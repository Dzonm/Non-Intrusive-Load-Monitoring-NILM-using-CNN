# -*- coding: utf-8 -*-
"""
Created on Fri Aug  2 08:42:14 2024
...
def find_test_filename(test_dir, appliance, test_type) -> str:
    ...
    for filename in os.listdir(os.path.join(test_dir, appliance)):
        if test_type == 'validation' and 'VALIDATION' in filename.upper():
...
if __name__ == '__main__':
    ...
    logger = Logger(
        log_file_name = os.path.join(
            args.save_dir, appliance_name, f'{appliance_name}_test_on_validation_processed_{args.model_arch}.log'
        )
    )
    ...
    test_filename = find_test_filename(
        args.datadir, appliance_name, 'validation'
    )
    ...
    logger.log(f'Saving ground truth, and processed_prediction on validation to {save_path}.')
    np.save(f'{save_path}/{test_filename}_pred_{args.model_arch}processed.npy', processed_prediction)
