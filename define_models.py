...
def lstm(window_length=599, lstm_units=128, dense_units=64, drop_rate=0.3) -> tf.keras.Model:
    """Specifies an LSTM-based seq2point model using the Keras Functional API.
    ...
    lstm_out = tf.keras.layers.Dropout(rate=drop_rate)(lstm_out)
    ...
    dense_out = tf.keras.layers.Dropout(rate=drop_rate)(dense_out)

    # Output layer
    output_layer = tf.keras.layers.Dense(units=1, activation='linear')(dense_out)

    return tf.keras.models.Model(inputs=input_layer, outputs=output_layer, name='lstm')
